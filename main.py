...
def train(train_dloader, valid_dloader, encoder, decoder, discriminator, enc_optimizer, dec_optimizer, dis_optimizer,
    ...
    for i, (real_image, index, *_) in enumerate(train_dloader):
        ...
        dis_loss_fake_inf = cls_criterion(output, fake_inf_label)
        ...
        dec_loss_inf = cls_criterion(output, dec_label_inf)
        ...
        discriminator_fake_loss.update(float(dis_loss_fake_gen + dis_loss_fake_inf) / 2)
        discriminator_real_loss.update(float(dis_loss_real))
        decoder_fake_loss.update(float(dec_loss_gen + dec_loss_inf) / 2)
